...
extern int TOKEN_LEN;
	...
	char* lexical_analyser::validate(char *token){
		if(is_delimeter(token[0])) return token;
		// Else it is either keyword, or identifier, or string literal, or numeric constant.
		if(is_keyword(token)) return token;
		else if(is_identifier(token)) return token;
		else if(is_string_literal(token)) return token;
		else if(is_numeric_constant(token)) return token;
		else{
